(function(){
  ...
  function preview(){config.apply(collect(),false);$('#syncNote').textContent='Draft preview active. Apply to sync the itinerary.'}
  ...
  document.addEventListener('designsystemchange',event=>{const detail=event.detail||{};if(detail.persist||detail.source==='storage')hydrate();renderGallery(detail.tokens||detail)});
