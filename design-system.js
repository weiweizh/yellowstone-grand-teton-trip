(function(){
  ...
  function updateFontLabels(){const style=getComputedStyle(document.documentElement);$$('[data-font-token]').forEach(node=>{const raw=style.getPropertyValue(node.dataset.fontToken)||'';node.textContent=raw.split(',')[0].replace(/['"]/g,'').trim()||'system default'})}
  function hydrate(){const current=config.read();colorControls.forEach(control=>{const token=control.dataset.colorControl;const value=current[token]||'#000000';const color=control.querySelector('input[type=color]');const hex=control.querySelector('.hex');if(/^#[0-9a-f]{6}$/i.test(value)){color.value=value;hex.value=value}});rangeControls.forEach(input=>{if(!input.dataset.token)return;const value=current[input.dataset.token];if(value){const number=parseFloat(value);if(Number.isFinite(number))input.value=number}const output=input.parentElement.querySelector('output');if(output)output.textContent=displayValue(input)});renderGallery(current);updateFontLabels()}
  ...
  function preview(){config.apply(collect(),false);updateFontLabels();$('#syncNote').textContent='Draft preview active. Apply to sync the itinerary.'}
