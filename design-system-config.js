(function(){
  ...
  const defaults={
    ...
    '--font-display':"'Amarna', sans-serif",'--font-body':"'Atkinson Hyperlegible Next', sans-serif",'--font-mono':"'DM Mono', monospace",
    '--text-body':'1rem','--text-lg':'1.125rem','--text-xl':'1.375rem','--text-2xl':'1.75rem','--text-3xl':'2.25rem','--text-4xl':'clamp(3rem, 8vw, 7rem)',
    '--space-xs':'8px','--space-sm':'12px','--space-md':'16px','--space-lg':'24px','--space-xl':'32px','--space-2xl':'48px','--space-3xl':'80px',
  ...
  function migrateSpacing(value){const match=/^([0-9.]+)rem$/.exec(String(value));if(!match)return value;const px=Math.round(parseFloat(match[1])*16/4)*4;return `${px}px`}
  function read(){try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');Object.keys(saved).forEach(key=>{if(key.startsWith('--space-')){const converted=migrateSpacing(saved[key]);if(converted!==saved[key]){saved[key]=converted}}});const fontMigrationKey='yellowstone-font-preset-v2';if(localStorage.getItem(fontMigrationKey)!=='done'){saved['--font-body']=defaults['--font-body'];localStorage.setItem(fontMigrationKey,'done')}if(String(saved['--font-display']||'').includes('Playfair Display'))saved['--font-display']=defaults['--font-display'];if(String(saved['--font-body']||'').includes('DM Sans'))saved['--font-body']=defaults['--font-body'];return {...defaults,...saved}}catch{return {...defaults}}}
